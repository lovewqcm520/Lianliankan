package com.jack.llk.view.module.time
{
	...
	public class TimeModelChapterView extends BaseView
	{
		...
		/**
		 * Start time game at specify level.
		 */
		private function onStartGameAt(level:int):void
		{
			var gameView:GameView=new GameView(Common.GAME_MODEL_TIME);
		...
		{
			// dispose the time model game view
		...
		private function updateChapterList():void
		{
			...
			var data:Array = [];			
			// get time model level data from cache
			TimeModelVO.getInstance().init();
			// get total chapters and chapter list
			var len:int=TimeModelVO.getInstance().nChapters;
			var list:Vector.<ChapterVO>=TimeModelVO.getInstance().chapterList;
