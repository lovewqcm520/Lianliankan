package com.jack.llk.vo.map
{
	...
	public class Map
	{
		...
		public function Map(w:int, h:int, availableItem:int, avalevel:uint = 5)
		{
			if(!NumberUtil.isEven(availableItem))
				Log.error(availableItem + " was not a even number.");
		...
		public function set level(value:uint):void
		{
			_level = value;
			//取得一个尽量大的偶数值
			_countOfPerItem = NumberUtil.getFloorEven(availableItem / _level);
			//_countOfPerItem = int(availableItem / _level);
			_restBlock = availableItem;
		...
		private function _initMap():void
		{
			...
			var left:int = availableItem - start;
			
			for (var k:int = start; k < availableItem; k+=2) 
			{
		...
		private function _findRestPointB(a:Point, ignore_b_arr:Array 
										 = null):Point
		{
			if (!a) return null;
